...
class LocalEPdfProcessor:
    ...
    def save_formatted_results(self, extracted_data_folder: Path, session_id: str, formatted_data: Dict[str, Any]) -> str:
        ...
        if "formatted_transactions" in formatted_data and formatted_data["formatted_transactions"]:
            ...
            # Map cheque_reference to 'chq./Ref.No.' and drop internal key
            if 'cheque_reference' in df.columns:
                df['chq./Ref.No.'] = df['cheque_reference'].fillna('')
                df = df.drop(columns=['cheque_reference'])
            else:
                # Ensure the column exists even if no data provided
                df['chq./Ref.No.'] = ''
            
            # Desired column order; only keep those present to avoid KeyError
            desired_order = ['date', 'mode', 'particulars', 'chq./Ref.No.', 'deposits', 'withdrawals', 'balance']
